...
# HÃ€M get_image_data Bá»Š Bá»Ž QUA HOáº¶C XÃ“A Bá»Ž Náº¾U KHÃ”NG Cáº¦N THIáº¾T.
# VÃŒ CHÃšNG TA CHá»ˆ QUÃ‰T THÆ¯ Má»¤C OUTPUT TRá»°C TIáº¾P.
...
def handler(job):
    ...
    Returns:
        dict: A dictionary containing either an error message or a success status with generated files (images/videos).
    """
    job_input = job["input"]
    job_id = job["id"]
    ...
    prompt_id = None
    # Thay Ä‘á»•i tÃªn biáº¿n tá»« output_data sang output_files_data
    output_files_data = [] # Biáº¿n má»›i Ä‘á»ƒ chá»©a táº¥t cáº£ output files (video/áº£nh)
    errors = []
    ...
    try:
        ...
        # --- Báº®T Äáº¦U PHáº¦N ÄÃƒ Sá»¬A Äá»”I: Xá»¬ LÃ HISTORY VÃ€ QUÃ‰T THÆ¯ Má»¤C OUTPUT ---

        # Fetch history (Ä‘á»ƒ kiá»ƒm tra lá»—i vÃ  tráº¡ng thÃ¡i job)
        ...
        # Láº¥y chi tiáº¿t output tá»« history (chá»‰ Ä‘á»ƒ kiá»ƒm tra lá»—i/cáº£nh bÃ¡o, KHÃ”NG Láº¤Y Dá»® LIá»†U FILE)
        prompt_history = history.get(prompt_id, {})
        outputs = prompt_history.get("outputs", {})

        if not outputs:
            # Váº«n cáº£nh bÃ¡o náº¿u khÃ´ng cÃ³ output trong history, nhÆ°ng sáº½ Ä‘Æ°á»£c ghi Ä‘Ã¨ bá»Ÿi viá»‡c quÃ©t file
            warning_msg = f"No outputs found in history for prompt {prompt_id}."
            print(f"worker-comfyui - {warning_msg}")
            #errors.append(warning_msg) # TÃ¹y chá»n: cÃ³ thá»ƒ thÃªm vÃ o errors náº¿u muá»‘n cáº£nh bÃ¡o nÃ y xuáº¥t hiá»‡n trong details

        print(f"worker-comfyui - Processing {len(outputs)} output nodes for error/warning check...")
        for node_id, node_output in outputs.items():
            # ChÃºng ta khÃ´ng cÃ²n láº¥y 'images' tá»« Ä‘Ã¢y ná»¯a.
            # Chá»‰ kiá»ƒm tra cÃ¡c keys khÃ¡c Ä‘á»ƒ cáº£nh bÃ¡o (vÃ­ dá»¥ 'gifs' mÃ  báº¡n Ä‘Ã£ tháº¥y)
            other_keys = [k for k in node_output.keys() if k != "images"]
            if other_keys:
                warn_msg = (
                    f"Node {node_id} produced unhandled output keys: {other_keys}. "
                    f"These outputs are not fetched via ComfyUI API, but will be collected if saved to /comfyui/output/."
                )
                print(f"worker-comfyui - WARNING: {warn_msg}")
                # errors.append(warn_msg) # TÃ¹y chá»n: cÃ³ thá»ƒ thÃªm vÃ o errors náº¿u muá»‘n cáº£nh bÃ¡o nÃ y xuáº¥t hiá»‡n trong details


        # --- Báº®T Äáº¦U PHáº¦N Má»šI: QUÃ‰T THÆ¯ Má»¤C OUTPUT TRá»°C TIáº¾P ---
        output_dir = "/comfyui/output/"
        if os.path.exists(output_dir):
            print(f"worker-comfyui - Scanning {output_dir} for output files...")
            # CÃ¡c Ä‘á»‹nh dáº¡ng file phá»• biáº¿n cho video vÃ  áº£nh
            file_extensions = ["mp4", "webm", "gif", "png", "jpg", "jpeg"]

            for ext in file_extensions:
                for file_path in glob.glob(os.path.join(output_dir, f"*.{ext}")):
                    try:
                        ...
                        print(f"worker-comfyui - Collected file: {os.path.basename(file_path)} (Type: {mime_type})")

                        # TÃ¹y chá»n: XÃ³a file sau khi thu tháº­p Ä‘á»ƒ giá»¯ sáº¡ch thÆ° má»¥c output
                        os.remove(file_path)
                        print(f"worker-comfyui - Removed collected file: {os.path.basename(file_path)}")

                    except Exception as e:
                        print(f"worker-comfyui - Error collecting file {file_path}: {e}")
                        errors.append(f"Error collecting file {os.path.basename(file_path)}: {e}")
            else:
                print(f"worker-comfyui - Output directory {output_dir} does not exist.")
    ...
    if output_files_data:
        final_result["output_files"] = output_files_data # Key má»›i Ä‘á»ƒ chá»©a táº¥t cáº£ output files
    
    if errors:
        final_result["errors"] = errors
        print(f"worker-comfyui - Job completed with errors/warnings: {errors}")

    if not output_files_data and errors:
        print(f"worker-comfyui - Job failed with no output files.")
        return {
            "error": "Job processing failed",
            "details": errors,
        }
    elif not output_files_data and not errors:
        print(
            f"worker-comfyui - Job completed successfully, but no output files were collected."
        )
        final_result["status"] = "success_no_outputs"
        final_result["output_files"] = []

    print(f"worker-comfyui - Job completed. Returning {len(output_files_data)} file(s).")
    return final_result
